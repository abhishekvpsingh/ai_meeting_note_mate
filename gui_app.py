...
class MeetingNoteMateApp:
    ...
    def _process_audio(self, audio_path):
        try:
            # Normalize the input — audio_path may be Path, str, or file object
            from pathlib import Path
            if hasattr(audio_path, "name"):
                path_display = Path(audio_path.name)
            else:
                path_display = Path(audio_path)

            print(f"🎧 Using audio file: {path_display}")

            # Pass full path (or filename) directly
